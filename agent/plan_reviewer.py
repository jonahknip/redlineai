...
Be thorough and specific. Note anything that appears incomplete or requires follow-up."""
    ...
    def generate_ai_report(self, use_vision: bool = True, checklist: dict = None, custom_instructions: str = "") -> str:
        ...
        # Debug logging
        print(f"[DEBUG] generate_ai_report() called")
        print(f"[DEBUG] OPENAI_AVAILABLE: {OPENAI_AVAILABLE}")
        print(f"[DEBUG] API Key present: {bool(api_key)}")
        print(f"[DEBUG] use_vision: {use_vision}")
        print(f"[DEBUG] checklist provided: {bool(checklist)}")
        if checklist:
            print(f"[DEBUG] checklist name: {checklist.get('name', 'N/A')}")
            print(f"[DEBUG] checklist sections: {len(checklist.get('sections', []))}")
        ...
        # Try to load training examples for few-shot learning
        few_shot_examples = ""
        try:
            from agent.training import get_training_store, generate_few_shot_prompt
...
- PASS = Clearly verified/met in the plans - provide specific evidence from the drawings
- FAIL = Not met, missing, or has issues - explain what is wrong or missing
...
Provide specific, actionable comments referencing what you observed. Return ONLY the JSON array."""
        ...
        print(f"[DEBUG] Building HTML report...")
        print(f"[DEBUG] Checklist items: {len(checklist_items_for_ai)}")
        print(f"[DEBUG] Evaluation results: PASS={counts['PASS']}, FAIL={counts['FAIL']}, REVIEW={counts['REVIEW']}, N/A={counts['N/A']}")
...
<div class="header">
<div class="company">REDLINE.AI</div>
...
<div class="footer">
Generated by Redline.ai | Full Drawing Analysis Platform | {review_date}
</div>
        ...
        print(f"[DEBUG] HTML report generated successfully, length: {len(html)} chars")
        return html
