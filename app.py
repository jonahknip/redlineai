...
def export_pdf():
    ...
    try:
        ...
        # Check if report already has full HTML structure
        if report_html.strip().startswith('<!DOCTYPE') or report_html.strip().startswith('<html'):
            # Report already has full HTML with styles - use it directly
            full_html = report_html
        else:
...
h2 {{ color: #1B365D; border-bottom: 1px solid #ddd; padding-bottom: 5px; margin-top: 25px; }}
...
def send_email():
    """Send report via email - opens Outlook with report content"""
    try:
        data = request.get_json()
        recipient = data.get('email', '')
        report_html = data.get('report', '')
        project_name = data.get('project_name', 'Planset Review')
        review_type = data.get('review_type', 'QA/QC')
        ...
        # Create clean plain text version for email body
        from bs4 import BeautifulSoup
        soup = BeautifulSoup(report_html, 'html.parser')
        ...
        subject = f"QA/QC Review Report: {project_name}"
        
        # Format email body with header
        body = f"""REDLINE.AI - QA/QC REVIEW REPORT
...
This report was generated by Redline.ai
AI-Powered QA/QC for Civil Engineering Design

Note: For the full formatted report with tables and styling, please use the "Export Word" option and attach the document to your email.
"""
        
        # Create mailto link (Outlook compatible)
